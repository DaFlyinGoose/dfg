...
class CronSubscribed extends Command {
	...
	{
		$count = EmailService::subscribersCron($this->option('pretend'));
		
		$return = ' users were subscribed';
		
		if ($count > 0)
		{
			if ($this->option('pretend') || Config::Get('mail.pretend'))
			{	
	...
	{
		return array(
			array('pretend', null, InputOption::VALUE_NONE , 'Disables the mail function whilst still ading users to the system', null),
		);
	}
